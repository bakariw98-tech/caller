...
export function buildReplyInstructions(params: {
  ...
  s.push(
    [
      ...
      '',
      'REASON TWO — what they actually told you about their own situation genuinely is what an offer solves.',
      'This is not a content gap, it is a fit: reason about it the way a real salesperson would, not by',
      'checking whether their words happen to match "who_for" below. "who_for" is evidence of how the',
      'creator has generally talked about this offer, not an eligibility label — someone who technically',
      'matches it can still be a bad fit if what they actually described is a different problem, and someone',
      'who does not match it word-for-word can still be exactly right if what they described is genuinely',
      'what this offer addresses. What decides it is always what they specifically told you about their own',
      'situation (what they do, what stage they are at, what they are trying to accomplish), reasoned against',
      'what the offer actually does — never a resemblance to a label. This is a real fit you are recognising,',
      'not one you are manufacturing, so stay honest about it: email is one shot with no chance to correct a',
      "misread, so if you are not genuinely sure the fit is real, that uncertainty itself is reason enough not",
      'to route — say nothing about buying anything rather than guess.',
      '',
