...
import type { Creator, Customer, Enrollment, Step } from '../domain/types.js';
import {
  findStepByPosition,
  getStep,
  matchProblems,
...
/**
 * Tool surface exposed to the coach.
 *
 * Descriptions are written for the model, in the same voice as the prompt: they
 * say when to reach for a tool, not just what it returns. None of them accepts
 * an identifier for the caller — see mcp/auth.ts.
 */
export const TOOL_DEFINITIONS: ToolDefinition[] = [
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        query: { type: 'string', description: "What to look for, in the caller's own words." },
      },
      required: ['query'],
      additionalProperties: false,
    },
...
export interface ToolContext {
  db: DB;
  session: McpSession;
  /** Injected by the call router so a transfer can act on the live call. */
  transferCall?: (targetE164: string) => Promise<void>;
  logEvent?: (type: string, payload: Record<string, unknown>, stepId?: string | null) => void;
}

export interface ToolResult {
  /** Serialised as the tool's text content. Tool output is exempt from billing. */
  data: unknown;
  isError?: boolean;
}
...
function loadEnrollment(ctx: ToolContext): Enrollment | null {
  if (!ctx.session.enrollment_id) return null;
  return (
    (ctx.db.prepare('SELECT * FROM enrollments WHERE id = ?').get(ctx.session.enrollment_id) as
      | Enrollment
      | undefined) ?? null
  );
}

function loadCustomer(ctx: ToolContext): Customer | null {
  if (!ctx.session.customer_id) return null;
  return (
    (ctx.db.prepare('SELECT * FROM customers WHERE id = ?').get(ctx.session.customer_id) as
      | Customer
      | undefined) ?? null
  );
}

/** Shapes a step for speech: prose fields, no ids, nothing the coach cannot say. */
function renderStep(detail: StepDetail) {
  return {
    ...
    common_problems: detail.problems.map((p) => ({
      symptom: p.symptom,
      cause: p.cause,
      fix: p.fix,
    })),
...
function getCallerState(ctx: ToolContext): ToolResult {
  const customer = loadCustomer(ctx);
  const enrollment = loadEnrollment(ctx);
  if (!customer || !enrollment) return { data: NOT_IDENTIFIED };

  const current = enrollment.current_step_id ? getStep(ctx.db, enrollment.current_step_id) : null;
  const open = openProblems(ctx.db, enrollment.id);
  const balance = balanceSeconds(ctx.db, customer.id, ctx.session.creator_id);
    ...
    data: {
      ...
      position: current
        ...
        : enrollment.status === 'completed'
          ...
          : 'Has not started yet',
      progress_summary: describeProgress(ctx.db, enrollment),
...
function getCurrentStep(ctx: ToolContext): ToolResult {
  const enrollment = loadEnrollment(ctx);
  ...
  const detail = getStep(ctx.db, enrollment.current_step_id);
  if (!detail) return { data: { message: 'That step is no longer in the current version of the course.' } };
  ctx.logEvent?.('step_viewed', { step: detail.step.title }, detail.step.id);
  return { data: renderStep(detail) };
}

function getStepByPosition(ctx: ToolContext, args: Record<string, unknown>): ToolResult {
  const pos = {
  ...
  const step = findStepByPosition(ctx.db, ctx.session.course_id, pos);
  if (!step) {
    return {
      data: {
        found: false,
  ...
  const detail = getStep(ctx.db, step.id)!;
  ctx.logEvent?.('step_viewed', { step: detail.step.title }, step.id);
  return { data: renderStep(detail) };
}

function doSearch(ctx: ToolContext, args: Record<string, unknown>): ToolResult {
  const query = typeof args.query === 'string' ? args.query : '';
  if (!query.trim()) return { data: { error: 'query is required' }, isError: true };

  const enrollment = loadEnrollment(ctx);
  const hits = searchCurriculum(ctx.db, ctx.session.course_id, query, {
    nearStepId: enrollment?.current_step_id ?? null,
    limit: 5,
  });

  ctx.logEvent?.('search', { query, hits: hits.length });
...
function doDiagnose(ctx: ToolContext, args: Record<string, unknown>): ToolResult {
  ...
  if (typeof args.module === 'number' || typeof args.step === 'number') {
    step = findStepByPosition(ctx.db, ctx.session.course_id, {
      module: typeof args.module === 'number' ? args.module : undefined,
      step: typeof args.step === 'number' ? args.step : undefined,
    });
  }
  if (!step && enrollment?.current_step_id) {
    step = (ctx.db.prepare('SELECT * FROM steps WHERE id = ?').get(enrollment.current_step_id) as Step) ?? null;
  }
  ...
  // Nothing at this step: widen to the course before giving up, but never past it.
  const hits = searchCurriculum(ctx.db, ctx.session.course_id, symptom, {
    nearStepId: step?.id ?? null,
    limit: 3,
  });
  if (hits.length > 0) {
    return {
...
function doRecordProgress(ctx: ToolContext, args: Record<string, unknown>): ToolResult {
  const enrollment = loadEnrollment(ctx);
  if (!enrollment) return { data: NOT_IDENTIFIED };
  ...
  switch (event) {
    case 'completed_step': {
      const { completed, advancedTo } = completeAndAdvance(ctx.db, enrollment, {
        callId: ctx.session.call_id,
        note,
      });
      ctx.logEvent?.('step_completed', { step: completed?.title ?? null }, completed?.id ?? null);
      return {
        data: {
          recorded: true,
          ...
          now_on: advancedTo
            ? advancedTo.title
            : 'Nothing — they have finished the last step of the course.',
        },
      };
    }
    ...
    case 'noted_preference': {
      ...
      if (event === 'hit_problem') {
        ctx.logEvent?.('problem_recorded', { problem }, currentStepId);
      }
...
async function doRequestHuman(ctx: ToolContext, args: Record<string, unknown>): Promise<ToolResult> {
  ...
  const creator = ctx.db.prepare('SELECT * FROM creators WHERE id = ?').get(ctx.session.creator_id) as
    | Creator
    | undefined;
  const enrollment = loadEnrollment(ctx);
  ...
  ctx.logEvent?.('escalation', { reason, transfer: wantsTransfer }, enrollment?.current_step_id ?? null);
  ...
  if (wantsTransfer && canTransfer) {
    try {
      ...
      return {
        data: {
          transferring: true,
          say: 'Let them know you are putting them through now, then stop speaking.',
        },
      };
    } catch (err) {
      return {
        data: {
          ...
          logged: true,
          say:
            'The transfer did not go through. Tell them their question has been passed to ' +
            `${creator?.business_name ?? 'the team'} and someone will follow up.`,
  ...
  return {
    data: {
      ...
      logged: true,
      say:
        `Their question has been passed to ${creator?.business_name ?? 'the team'}. Tell them someone will ` +
        'follow up, and give them whatever the curriculum does cover in the meantime.',
    },
  };
}
